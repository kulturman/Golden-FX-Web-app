...
router.get('/user' , auth , async (req , res) => {
    const variations = await UserFundVariation.findAll({
        ...
        include: [
            { model : FundVariation , as : 'variation' }
        ]
    });
    return res.send(variations);
})
...
router.get('/graphData/:date/:type' , auth , async (req , res) => {
    ...
    const data = type === 'day' ? await FundVariation.getDataForGraphByDays(date)
        : await FundVariation.getDataForGraphByMonths(date);
    return res.send(data);
})

router.post('/' , [ auth , isAdmin , fundVariationValidator() ] , async (req , res) => {
    ...
    /* si le fonds avait subit une perte, on comble d'abord la perte et le reste éventuel constitue un bénéfice */
    else if (misc.fundCurrentAmount < misc.fundAmount) {
        ...
        if(gain + misc.fundCurrentAmount >= misc.fundAmount) {
            const realGain = gain - (misc.fundAmount - misc.fundCurrentAmount);
            variation.realPercentage = (realGain / misc.fundCurrentAmount) * 100
    ...
    try {
        const createdVaritaion = await FundVariation.create(variation , { transaction });
